  ...
  return (
    <div className="flex w-2/5 min-h-[170px] p-2 flex-col justify-between items-center text-center gap-3">
      ...
      <div className="text-wrap">
        <Link href={`/events?srch=${undefined}&location=${undefined}&category=${category}`} className="font-opensans font-semibold text-[24px] text-[#2D2C3C] hover:underline">{title}</Link>
      </div>
    </div>
  )
}